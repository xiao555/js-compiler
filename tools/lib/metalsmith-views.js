...
var debug = require('debug')('metalsmith-views');
 ...
 * Settings
 *
 * Options supported by metalsmith-views
 */
var settings = [
  'default',
  'directory',
...
function plugin(opts){
  ...
  return function(files, metalsmith, done){
    ...
    function convert(file, done){
      ...
      var clone = extend({}, clonedParams, metadata, {post: data});
      var str = metalsmith.path(dir, layout || def);
      ...
      render(str, clone, function(err, str){
        ...
        cloneData.contents = new Buffer(str);
        debug('converted file: %s', file);

        files[file] = cloneData;
        done();
      });
    }
